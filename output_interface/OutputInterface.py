...
class OutputInterface:
            ...
            for shell_i, shell in enumerate(basis_shells):
                if shell == 'S':
                    ...
                    for combi in self.get_combinations(shell):
                        # Find the values of i,j,k for the specific combination - a bit stupid way, but then it looks like the output file...
                        i, j, k = (0, 0, 0)
                        for val in combi:
                            if val == 'X':
                                i += 1
    ...
    def export_GTOs(self, path=None, orbital_nr=None):
        ...
        #with open(f'{name}_GTOs.txt', 'w') as file:
        #    # Describe the output here...!
        #    for line in output:
        #        print(line)
        #        file.write(str(line)[1:-1] + "\n")

    def eval_orbital(self, x, y, z, orbital_nr):
